#!/bin/bash
# G3 official 30-run + G4 paired evidence for the 7-epoch record candidate.
# Same-node paired: champ7 (frontier-record recipe reproduction) vs s3p_7
# (champ + ResNet-D + Lookahead + BN-beta LR + progressive-resize). Official
# protocol: 30 runs, SLEEP_CYCLES=1e9, official seeds sb=880000, target 0.70.
#SBATCH --job-name=c100-g3rec-dcrisost
...
#SBATCH --output=./logs/g3rec-%j.out
#SBATCH --error=./logs/g3rec-%j.err
set -uo pipefail
...
# Official protocol (30 runs, thermal sleep on, official seed base, target 0.70).
