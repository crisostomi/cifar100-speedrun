...
CIFAR_MEAN = torch.tensor((0.5071, 0.4867, 0.4408), dtype=torch.half)
CIFAR_STD = torch.tensor((0.2675, 0.2565, 0.2761), dtype=torch.half)
...
def main(run, model):
    ...
    test_loader = CifarLoader("cifar100", train=False, batch_size=2000)
    ...
    if run == "warmup":
        train_loader.labels = torch.randint(
            0, 100, size=(len(train_loader.labels),), device=train_loader.labels.device
        )
        ...
        test_loader.labels = torch.randint(
            0, 100, size=(len(test_loader.labels),), device=test_loader.labels.device
        )
        test_loader.images = torch.randn_like(
            test_loader.images, device=test_loader.images.device
        )
    total_train_steps = ceil(float(os.getenv("C100_EPOCHS", "24")) * len(train_loader))
...
if __name__ == "__main__":
    ...
    main("warmup", model)
    results = []
    for run in range(int(os.getenv("C100_RUNS", "50"))):
        torch.cuda.empty_cache()
        torch.cuda.synchronize()
        torch.cuda._sleep(int(os.getenv("C100_SLEEP_CYCLES", "1000000000")))
